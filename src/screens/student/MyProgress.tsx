...
export default function MyProgress() {
  ...
  const load = useCallback(async () => {
    if (!user) return;
    const r = await getMemorizationByStudent(user.id);
    setRecords(r);
  }, [user]);
    ...
    <ScrollView
      ...
      <View style={styles.summaryRow}>
        <View style={styles.summaryCard}>
          ...
          <Text style={styles.sumLabel}>سور تم حفظها</Text>
        </View>
        <View style={styles.summaryCard}>
...
const styles = StyleSheet.create({
  ...
  gradeSummary: {flexDirection: 'row', gap: 6},
  ...
  recordSurah: {fontSize: 14, color: COLORS.text, textAlign: 'right'},
